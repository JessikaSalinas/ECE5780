...
void PI_update(void) {		
		 ...
		 *       more resolution.
		 */
		// motor speed in rpm = motor_speed * (1 rotation / 3200 pulses) * (60 sec / 1 min) 
		error = (target_rpm * 2) - motor_speed;	 // Gets error signal

		/// TODO: Calculate integral portion of PI controller, write to "error_integral" variable
		error_integral += error;     // Integral is the summation of error * 45ms period (45/1000)
		 ...
		 */
		 	// Clamp integral value between 0 and 3200
		if (error_integral < 0) {
			error_integral = 0;     // If below clamp range, just set to 0
		}
		else if (error_integral > 3200) {
			error_integral = 3200;  // If above clamp range, just set to 3200
		}

		/// TODO: Calculate proportional portion, add integral and write to "output" variable	
		int16_t proportional = Kp * error;							// Prop. gain * error
		int16_t integral     = Ki * error_integral;			// Integ. gain * error integ.
		int16_t output       = proportional + integral; // Calculated PI output
		 ...
		 */
		 ...
		 if (output < 0) {
			output = 0;     // If below clamp range, just set to 0
		}
		else if (output > 100) {
			output = 100;   // If above clamp range, just set to 100
		}
