...
int main(void)
{
  ...
  __HAL_RCC_GPIOA_CLK_ENABLE(); // Enable peripheral GPIOA clock (for button)
  ...
  //MX_USB_PCD_Init();
	...
	//EXTI->IMR |= (1<<0); // Set bit 0 of EXTI0 for PA0 (write bit 1)
	EXTI->IMR = 0x0001; // Set bit 0 of EXTI0 for PA0 (write bit 1)
	
	// Configure the EXTI input line 0 to have a rising-edge trigger
	//EXTI->RTSR |= (1<<0);  // Set bit 0 of EXTI0 for PA0 (write bit 1)
	EXTI->RTSR = 0x0001; // Set bit 0 of EXTI0 for PA0 (write bit 1)
	//EXTI->FTSR &= ~(1<<0); // Disable Falling Edge Trigger for PA0
	
	// Configure the SYSCFG multiplexer to route PA0 to the EXTI input line 0 (EXTI0)
	SYSCFG->EXTICR[0] &= (uint16_t)~SYSCFG_EXTICR1_EXTI0_PA; // Write bits x000 for EXIO PA0
	
	//NVIC_SetPriority (EXTI0_1_IRQn, 1); // Set priority of EXTI0 to 1
	NVIC_SetPriority (EXTI0_1_IRQn, 3); // Set priority of EXTI0 to 3
	NVIC_EnableIRQ (EXTI0_1_IRQn);      // Enable interrupt for EXTI0
	
	// Change the SysTick interrupt priority to 2 (medium priority)
	NVIC_SetPriority (SysTick_IRQn, 2); // Set priority of SysTick to 2
		
	while (1)
  {
		// Toggle the red LED (PC6) with a moderately-slow delay (400-600ms) in loop
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET); // Set PC6 high
